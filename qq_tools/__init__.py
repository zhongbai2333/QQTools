...
def on_load(server: PluginServerInterface, _):
    global __mcdr_server, config, data, help_info, online_players, admin_help_info, wait_list, debug_json_mode, debug_status, admins_command, time1, time2, old_send_id, start_time1, server_first_start
    ...
    admins_command = server.load_config_simple(
        "AdminCommand.json", target_class=AdminCommands
    )
    ...
    if not config.auto_forwards["mc_to_qq"]:
        ...
        server.register_command(
            Literal(":").then(GreedyText("message").runs(command_qq))
        )
    ...
    if not config.mysql_enable:  # 未开启mysql功能
        ...
        MySQL_Control.create_table_if_not_exists(
            "user_list",
            "id INT AUTO_INCREMENT PRIMARY KEY,qq_id VARCHAR(15),"
        ...
        __mcdr_server.logger.info(
            "QQTools无法启动，请安装mysql-connector-python或关闭数据库功能！"
        )
...
def on_server_startup(_):
    ...
    if config.mysql_enable and config.whitelist_add_with_bound:
        db_player_list = connect_and_query_db(
            "player_id", "user_list", config.mysql_config
        )
    ...
    if config.forwards_server_start_and_stop:
        ...
        if config.auto_forwards["qq_to_mc"]:  # 检测服务器是否自动转发QQ信息
            if server_first_start:
                msg_start = (
                    config.server_name
                    + f" 启动完成，用时 {start_time2 - start_time1} 秒！服务器已启用自动转发QQ信息！"
                )
                server_first_start = False
            else:
                msg_start = (
                    config.server_name
                    + f" 服务器核心重启完成！服务器已启用自动转发QQ信息！"
                )
        else:
            if server_first_start:
                msg_start = (
                    config.server_name
                    + f" 启动完成，用时 {start_time2 - start_time1} 秒！服务器已启用手动转发QQ信息！"
                )
                server_first_start = False
            else:
                msg_start = (
                    config.server_name
                    + f" 服务器核心重启完成！服务器已启用手动转发QQ信息！"
                )
...
def parse_msg(get_json):
    if (
        ...
        if msg[0] == "#":  # 分辨命令消息
            ...
            not config.auto_forwards["qq_to_mc"] and msg[0] == ":"
        ):  # 确认是否开启手动转发以及是否使用命令
            if msg[2:] != "":  # 检测命令语句是否合法
                user_list = get_user_list()
                if (
                    not str(get_json["user_id"]) in user_list.keys()
                ):  # 检测玩家是否已绑定
                    send_group_qq(
                        get_json["group_id"],
                    ...
                    __mcdr_server.say(
                        f"§7[QQ][{user_list[send_id]}] {msg[2:]}"
                    )  # 删除命令部分并转发消息
            else:
                send_group_qq(
                    get_json["group_id"], "错误的格式，请使用 : <msg>"
                )  # 错误提示
            ...
            if str(get_json["user_id"]) in user_list.keys():  # 检测是否绑定
                ...
                not str(get_json["user_id"]) in user_list.keys() and config.main_server
            ):
                send_group_qq(
                    get_json["group_id"],
                    f"[CQ:at,qq={send_id}] 在绑定 ID 前无法互通消息，请使用 #bound <ID> 绑定游戏ID",
    ...
    if get_json["notice_type"] == "group_increase":  # 进群处理
        ...
        if config.whitelist_remove_with_leave:  # 是否自动删除白名单
            if send_id in user_list.keys():  # 确认是否绑定过
                ...
                if config.main_server:
                    send_group_qq(
                        get_json["group_id"], f"{send_id} 已退群，此人未在服务器绑定"
                    )
        else:
            if send_id in user_list.keys():
                if config.main_server:
                    send_group_qq(
                        get_json["group_id"], f"{user_list[send_id]}({send_id}) 已退群"
                    )
                delete_user(send_id)
            else:
                if config.main_server:
...
def pares_private_command(send_id: str, command: str):
    ...
    # list 命令
        ...
        return (
            f"{config.server_name} 在线玩家共{len(online_players)}人，"
            ...
            if command[1] == "list" and len(command) == 2:
                ...
                if command[2] == "player":
                    if command[3] in user_list.values():
                        player_id_qq = list(user_list.keys())[
                            list(user_list.values()).index(command[3])
                        ]
                        if config.whitelist_add_with_bound:
    ...
    # tomcdr 命令
        ...
        if send_id in str(config.admins):
            ...
            answer = ""
            __mcdr_server.logger.info(
                f"[{user_list[send_id]}] >> {' '.join(command[1:])}"
            )
            __mcdr_server.execute_command(
                " ".join(command[1:]), RobotCommandSource(__mcdr_server, "QQBot")
            )
...
def pares_group_command(send_id: str, command: str):
    ...
    # bound 命令
        ...
        if send_id in user_list.keys():  # 检测玩家是否已经绑定
            ...
            if config.main_server:  # 确认服务器是否需要回复
                player_id_qq = list(user_list.keys())[
                    list(user_list.values()).index(command[1])
                ]
    ...
    # debug_json 命令
        ...
        command[0] == "debug_json"
        and debug_status
        and send_id not in str(config.admins)
    ):
        return "抱歉您不是管理员，无权使用该命令！"
...
def send_group_qq(gid: int, msg: str):
    if msg:
        ...
        try:
            res = requests.post(
                url="http://{0}:{1}/send_group_msg".format(
            ...
            if debug_status:
                __mcdr_server.logger.info(res.json())
        except requests.exceptions.ConnectionError:
            __mcdr_server.logger.warning(
                f"Can't found cq-gohttp Server,The message: {msg}({gid})"
            )
...
def send_private_qq(uid: int, msg: str):
    if msg:
        ...
        try:
            res = requests.post(
                url="http://{0}:{1}/send_private_msg".format(
                    config.send_host, config.send_port
                ),
                data=payload,
                headers=headers,
            ...
            __mcdr_server.logger.warning(
                f"Can't found cq-gohttp Server,The message: {msg}({uid})"
            )
...
def send_execute_mc(command: str):
    if __mcdr_server.is_server_running():  # 确认服务器是否启动
        ...
        __mcdr_server.logger.info(
            f"QQTools can't execute:{command}, The list of commands:{str(wait_list)}"
        )
...
def get_user_list():
    if config.mysql_enable:
        return dict(
            connect_and_query_db("qq_id,player_id", "user_list", config.mysql_config)
        )
    else:
        return data
...
def delete_user(send_id: str):
    if config.mysql_enable:
        if config.main_server:
            connect_and_delete_data(
                "user_list", f"qq_id = {send_id}", config.mysql_config
            )
    else:
        del data[send_id]
    ...
    if not re.search(pattern, name):
        if config.mysql_enable:
            if config.main_server:
                ...
                connect_and_insert_db(
                    "qq_id,player_id", "user_list", db_data, config.mysql_config
                )
                return True
            return True
        else:
